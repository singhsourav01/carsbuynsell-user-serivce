...
class ListingService {
  ...
  private populateSignedUrls = async (listings: any[]) => {
    ...
    try {
      ...
      for (const listing of listings) {
        ...
        if (listing.seller?.user_profile_image_file_id) {
          const signedUrl = fileMap.get(
            listing.seller.user_profile_image_file_id,
          );
          if (signedUrl) {
  ...
  getById = async (lst_id: string) => {
    ...
    if (!listing)
      throw new ApiError(
        ...
        LISTING_ERRORS.LISTING_NOT_FOUND,
      );
  ...
  create = async (seller_id: string, dto: CreateListingDTO) => {
    ...
    if (dto.lst_type === "AUCTION") {
      if (!dto.lst_auction_end)
        throw new ApiError(
          ...
          "Auction end date is required for auction listings",
        );
    ...
    const notifiy = await notifyNewListing(
      listing.lst_id,
      dto.lst_title,
      dto.lst_type,
    ).catch(() => {});
  ...
  update = async (lst_id: string, seller_id: string, dto: UpdateListingDTO) => {
    ...
    if (dto.lst_description !== undefined)
      data.lst_description = dto.lst_description;
    if (dto.lst_price !== undefined) data.lst_price = dto.lst_price;
    if (dto.lst_status !== undefined) data.lst_status = dto.lst_status;
    if (dto.lst_category_id !== undefined)
      data.lst_category_id = dto.lst_category_id;
    if (dto.lst_type !== undefined) data.lst_type = dto.lst_type;
    if (dto.lst_auction_end !== undefined)
      data.lst_auction_end = new Date(dto.lst_auction_end);
    if (dto.lst_min_increment !== undefined)
      data.lst_min_increment = dto.lst_min_increment;
    ...
    if (dto.lst_type === "AUCTION") {
      ...
      data.lst_min_increment =
        dto.lst_min_increment ?? listing.lst_min_increment ?? 1;
    }
  ...
  getMyListings = async (seller_id: string, page: number, take: number) => {
    const result = await this.listingRepository.findBySellerId(
      seller_id,
      page,
      take,
    );
  ...
  getListingByCategoryId = async (
    cat_id: string,
    page: number,
    take: number,
  ) => {
    const result = await this.listingRepository.getListingByCategoryId(
      cat_id,
      page,
      take,
    );
  ...
  getAuctionWithParticipants = async (lst_id: string) => {
    const auction =
      await this.listingRepository.findAuctionWithParticipants(lst_id);
    if (!auction)
      throw new ApiError(
        StatusCodes.NOT_FOUND,
        LISTING_ERRORS.LISTING_NOT_FOUND,
    ...
    if (auction.lst_type !== "AUCTION")
      throw new ApiError(
        StatusCodes.BAD_REQUEST,
        LISTING_ERRORS.LISTING_NOT_AUCTION,
      );
    ...
    const participants = auction.engagements.map((eng: any) => {
      const userBids = auction.bids.filter(
        (b: any) => b.bidder.user_id === eng.user.user_id,
      );
      const highestBid =
        userBids.length > 0
          ? Math.max(...userBids.map((b: any) => Number(b.bid_amount)))
          : 0;
  ...
  getBuyNowById = async (lst_id: string) => {
    ...
    if (listing.lst_type !== "BUY_NOW")
      throw new ApiError(
        StatusCodes.BAD_REQUEST,
        LISTING_ERRORS.LISTING_NOT_BUY_NOW,
      );
